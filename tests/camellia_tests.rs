...
//     let actual_plaintext = cipher.mbedtls_camellia_crypt_cfb128(iv, 16, ciphertext);
...
fn encrypt_ctr_128() {
    ...
    let actual_ct = cipher.mbedtls_camellia_crypt_ctr(16, &mut offset, nonce_counter, text);

    assert_eq!(actual_ct, expected_ciphertext);
}
...
fn decrypt_ctr_128() {
    ...
    let actual_pt = cipher.mbedtls_camellia_crypt_ctr(16, &mut offset, nonce_counter, ciphertext);

    assert_eq!(actual_pt, expected_text);
}

#[test]
fn encrypt_ctr_32bytes() {
    let keybits: u32 = 128;
    ...
    let actual_ct = cipher.mbedtls_camellia_crypt_ctr(32, &mut offset, nonce_counter, text);

    assert_eq!(actual_ct, expected_ciphertext);
}

#[test]
fn decrypt_ctr_32bytes() {
    let keybits: u32 = 128;
    ...
    let actual_pt = cipher.mbedtls_camellia_crypt_ctr(32, &mut offset, nonce_counter, ciphertext);

    assert_eq!(actual_pt, expected_text);
}

#[test]
fn encrypt_ctr_36bytes() {
    let keybits: u32 = 128;
    ...
    let expected_ciphertext: Vec<u8> = vec![
        0xB1, 0x9D, 0x1F, 0xCD, 0xCB, 0x75, 0xEB, 0x88, 0x2F, 0x84, 0x9C, 0xE2, 0x4D, 0x85, 0xCF,
        0x73, 0x9C, 0xE6, 0x4B, 0x2B, 0x5C, 0x9D, 0x73, 0xF1, 0x4F, 0x2D, 0x5D, 0x9D, 0xCE, 0x98,
        0x89, 0xCD, 0xDF, 0x50, 0x86, 0x96,
    ];
    ...
    let actual_ct = cipher.mbedtls_camellia_crypt_ctr(36, &mut offset, nonce_counter, text);

    assert_eq!(actual_ct, expected_ciphertext);
}

#[test]
fn decrypt_ctr_36bytes() {
    let keybits: u32 = 128;
    ...
    let actual_pt = cipher.mbedtls_camellia_crypt_ctr(36, &mut offset, nonce_counter, ciphertext);
