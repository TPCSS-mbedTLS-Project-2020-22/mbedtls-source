...
const chacha_test_keys:[[u8;32];2] = [
    [
...
const chacha_test_nonces:[[u8;12];2]=[
    [
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00
    ],
...
const chacha_test_counters:[u32;2]=[0,1];

 

const chacha_test_lengths:[usize;2]=[64,375];
...
fn mbedtls_chacha20_self_test(verbose:i32)->i32{
    ...
    let mut ret:i32= chacha20_header::MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    for i in 0..2{
        output=vec![];
        if verbose != 0{
            println!("Chacha20 test {}",i);
        }
        ret= chacha20::mbedtls_chacha20_crypt(
            chacha_test_keys[i],
            chacha_test_nonces[i],
            chacha_test_counters[i],
            chacha_test_lengths[i],
            test_input[i].clone(),
            &mut output
        );
        if ret!=0{
