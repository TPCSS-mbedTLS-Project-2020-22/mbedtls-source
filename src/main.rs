/* Note:------ Errors in this file are due to the dependencies on the files those are not implemented yet. ----------*/


mod gcmc;

use crate::gcmc::mbedtls_gcm_init;
//use crate::gcmc::gcm_gen_table;
use crate::gcmc::mbedtls_gcm_setkey;
//use crate::gcmc::gcm_mult;
use crate::gcmc::mbedtls_gcm_starts;
use crate::gcmc::mbedtls_gcm_update;
use crate::gcmc::mbedtls_gcm_finish;
use crate::gcmc::mbedtls_gcm_crypt_and_tag;
//use crate::gcmc::mbedtls_gcm_auth_decrypt;
use crate::gcmc::mbedtls_gcm_free;

use gcmc::gcm;
//use gcm::MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
use gcm::MBEDTLS_GCM_ENCRYPT;
use gcm::MBEDTLS_ERR_PLATFORM_FEATURE_UNSUPPORTED;
//use gcm::MBEDTLS_GCM_DECRYPT;
//use gcm::MBEDTLS_ERR_GCM_AUTH_FAILED;
//use gcm::MBEDTLS_ERR_GCM_BAD_INPUT;
//use gcm::MBEDTLS_ERR_GCM_HW_ACCEL_FAILED;
use gcm::mbedtls_gcm_context;

pub const MAX_TESTS: usize = 6;
pub const MBEDTLS_CIPHER_ID_AES: usize = 2;
...
pub const tag_test_data: [[u8;16];MAX_TESTS * 3] = [
    ...
    for j  in 0..3{
        ...
        for i in 0..MAX_TESTS{
            ...
            ret = mbedtls_gcm_setkey(&mut ctx, cipher, &mut temp , key_len as u32);
             ...
             */
             ...
             ret = mbedtls_gcm_crypt_and_tag(&mut ctx, MBEDTLS_GCM_ENCRYPT as i32, pt_len_test_data[i], &iv_test_data[iv_index_test_data[i] as usize], iv_len_test_data[i],&additional_test_data[add_index_test_data[i] as usize],add_len_test_data[i] , &pt_test_data[pt_index_test_data[i]as usize],&mut buf,16,&mut tag_buf);
            ...
            // }
             ...
             ret = mbedtls_gcm_setkey(&mut ctx, cipher, &key_test_data[key_index_test_data[i] as usize], key_len as u32);
             ...
             ret = mbedtls_gcm_crypt_and_tag(&mut ctx, MBEDTLS_GCM_ENCRYPT as i32, pt_len_test_data[i] , &mut iv_test_data[iv_index_test_data[i] as usize], iv_len_test_data[i], &mut additional_test_data[add_index_test_data[i] as usize], add_len_test_data[i], &mut ct_test_data[k], &mut buf, 16,&mut tag_buf);
            ...
            // }
             ...
             ret = mbedtls_gcm_setkey(ctx, cipher, &key_test_data[key_index_test_data[i] as usize], key_len as u32 );
             ...
             ret = mbedtls_gcm_starts(&mut ctx, MBEDTLS_GCM_ENCRYPT as i32, &mut iv_test_data[iv_index_test_data[i] as usize], iv_len_test_data[i], &mut additional_test_data[add_index_test_data[i] as usize], add_len_test_data[i]);
             ...
             if pt_len_test_data[i] > 32{
                 ...
                 ret = mbedtls_gcm_update(&mut ctx, rest_len as u64,&mut pt_test_data[pt_index_test_data[i] as usize][32..],  &mut buf[32..] );
             ...
             {
                 ret = mbedtls_gcm_update(&mut ctx, pt_len_test_data[i] as u64, &mut ct_test_data[k],&mut buf);
