...
pub fn put_uint32_be(n :  u32, b :&mut [u8;MBEDTLS_BLOWFISH_BLOCKSIZE],i: usize){
...
pub fn F(ctx : &mut mbedtls_blowfish_context, x:u32)->u32{
    let a:usize;
    let b:usize;
    let c:usize;
    let d:usize;
    let mut y:u64;
    let mut new_x:u32=x;
    d=usize::try_from(new_x & 0xFF).unwrap();
    new_x>>=8;
    c=usize::try_from(new_x & 0xFF).unwrap();
    new_x>>=8;
    b=usize::try_from(new_x & 0xFF).unwrap();
    new_x>>=8;
    a=usize::try_from(new_x & 0xFF).unwrap();
    //println!("{} {} {} {}",a,b,(*ctx).S[0][a],(*ctx).S[1][b]);
    y=(u64::from((*ctx).S[0][a])+u64::from((*ctx).S[1][b]))%MODULU32;
    y = y ^ u64::from((*ctx).S[2][c]);
    y = (y + u64::from((*ctx).S[3][d]))%MODULU32; 
    
    return u32::try_from(y).unwrap();
}
pub fn blowfish_enc(mut ctx : &mut mbedtls_blowfish_context,xl : &mut u32,xr : &mut u32){
    let mut Xl:u32;
    let mut Xr:u32;
    let mut temp:u32;
    Xl=*xl;
    Xr=*xr;
    for i in 0..MBEDTLS_BLOWFISH_ROUNDS{
        Xl=Xl ^ (*ctx).P[i];
        Xr = F( &mut ctx, Xl ) ^ Xr;
...
pub fn blowfish_dec(mut ctx : &mut mbedtls_blowfish_context,xl : &mut u32,xr : &mut u32){
    let mut Xl:u32;
    let mut Xr:u32;
    let mut temp:u32;
    Xl=*xl;
    Xr=*xr;
    for i in (2..MBEDTLS_BLOWFISH_ROUNDS+2).rev(){
        Xl = Xl ^ (*ctx).P[i];
        Xr = F( &mut ctx, Xl ) ^ Xr;
...
pub fn mbedtls_blowfish_setkey(mut ctx : &mut mbedtls_blowfish_context, key:&str,mut keybits: u32)->i32{
    ...
    (*ctx).S=S;
    j=0;
    for i in 0..MBEDTLS_BLOWFISH_ROUNDS+2 {
        data = 0x00000000;
        for _k in 0..4{
            data = ( data << 8 ) | u32::from(key.as_bytes()[j]);
            j+=1;
            if j>=keybits.try_into().unwrap() {
                j=0;
            }
        }
        (*ctx).P[i] = P[i] ^ data;
    ...
    let mut i:usize=0;
    while i < MBEDTLS_BLOWFISH_ROUNDS+2 {
        blowfish_enc( &mut ctx, &mut datal, &mut datar );
        (*ctx).P[i] = datal;
        (*ctx).P[i + 1] = datar;
        i+=2;
    }
...
pub fn mbedtls_blowfish_crypt_ecb(mut ctx : &mut mbedtls_blowfish_context,mode:usize, mut input: [u8;MBEDTLS_BLOWFISH_BLOCKSIZE], mut output : &mut [u8;MBEDTLS_BLOWFISH_BLOCKSIZE] )->i32{
    ...
    get_uint32_be(&mut X1,&mut input,4);

    if mode == MBEDTLS_BLOWFISH_DECRYPT {
        blowfish_dec( &mut ctx, &mut X0, &mut X1 );
    }

    else /* MBEDTLS_BLOWFISH_ENCRYPT */
    {
        blowfish_enc( &mut ctx, &mut X0, &mut X1 );
    }

    put_uint32_be( X0,&mut output,0);
    put_uint32_be( X1,&mut output,4);
